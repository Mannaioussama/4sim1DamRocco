...
final class AuthStore: ObservableObject {
    ...
    func login(email: String, password: String, rememberMe: Bool = true) async throws {
        ...
        if rememberMe {
            ...
            // Ensure no stale token remains in persistent storage when the user opts out
            try? tokenStore.remove()
        }

        // Mirror token so ActivityAPIService and other services can read it for this session
        AuthTokenManager.shared.saveToken(resp.accessToken)
        if let id = resp.user.id {
            AuthTokenManager.shared.saveUserId(id)
        }
