...
struct AICoachResponse {
    let suggestions: [LegacyAISuggestion]
    let tips: [AITip]
...
// Legacy suggestion model used only inside Gemini parsing.
// Renamed to avoid clashing with the new backend AISuggestion model.
...
class GeminiAIService: ObservableObject {
    ...
    private func parseGeminiResponse(_ response: String) -> AICoachResponse {
        var suggestions: [LegacyAISuggestion] = []
        var tips: [AITip] = []
    ...
    private func parseSuggestionLine(_ line: String) -> LegacyAISuggestion? {
    ...
    // MARK: - Fallback Data
    private func getDefaultSuggestions() -> [LegacyAISuggestion] {
        return [
            LegacyAISuggestion(
                title: "Morning Walk",
